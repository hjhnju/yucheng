...
(function () {
    ...
    extend(Helper.prototype, {
        ...
        removeClass: function (className) {
            var regexp = new RegExp('(^|\\s+)' + className + '(\\s+|$)'), i = this.length, element;
            while (i) {
                i -= 1;
                element = this[i];
        ...
        on: function (eventName, handler) {
            var eventNames = eventName.split(/\s+/), i, element;
    ...
    if (typeof define === 'function' && define.amd) {
        define('common/extra/Gallery/js/blueimp-helper', [], function () {
            return Helper;
        });
    } else {
...
(function (factory) {
    'use strict';
    if (typeof define === 'function' && define.amd) {
        define('common/extra/Gallery/js/blueimp-gallery', ['./blueimp-helper'], factory);
    } else {
        window.blueimp = window.blueimp || {};
    ...
    $.extend(Gallery.prototype, {
        ...
        support: function (element) {
            var support = { touch: window.ontouchstart !== undefined || window.DocumentTouch && document instanceof DocumentTouch }, transitions = {
                    webkitTransition: {
                    ...
                    var transition = support.transition, prop, translateZ;
        ...
        slide: function (to, speed) {
            ...
            var index = this.index, direction, naturalDirection, diff;
        ...
        close: function () {
            var that = this, closeHandler = function (event) {
        ...
        translate: function (index, x, y, speed) {
            var style = this.slides[index].style, transition = this.support.transition, transform = this.support.transform;
        ...
        animate: function (from, to, speed) {
            ...
            var that = this, start = new Date().getTime(), timer = window.setInterval(function () {
        ...
        onmouseout: function (event) {
            if (this.touchStart) {
                var target = event.target, related = event.relatedTarget;
        ...
        ontouchmove: function (event) {
            ...
            var touches = (event.originalEvent || event).touches[0], scale = (event.originalEvent || event).scale, index = this.index, touchDeltaX, indices;
        ...
        oncomplete: function (event) {
            var target = event.target || event.srcElement, parent = target && target.parentNode, index;
            if (!target || !parent) {
                return;
            }
        ...
        handleClick: function (event) {
            var options = this.options, target = event.target || event.srcElement, parent = target.parentNode, isTarget = function (className) {
        ...
        setTitle: function (index) {
            var text = this.slides[index].firstChild.title, titleElement = this.titleElement;
        ...
        imageFactory: function (obj, callback) {
            var that = this, img = this.imagePrototype.cloneNode(false), url = obj, backgroundSize = this.options.stretchImages, called, element, callbackWrapper = function (event) {
                    if (!called) {
                        event = {
                            type: event.type,
                            target: element
        ...
        createElement: function (obj, callback) {
            var type = obj && this.getItemProperty(obj, this.options.typeProperty), factory = type && this[type.split('/')[0] + 'Factory'] || this.imageFactory, element = obj && factory.call(this, obj, callback);
            if (!element) {
                element = this.elementPrototype.cloneNode(false);
        ...
        loadElements: function (index) {
            var limit = Math.min(this.num, this.options.preloadRange * 2 + 1), j = index, i;
            for (i = 0; i < limit; i += 1) {
                j += i * (i % 2 === 0 ? -1 : 1);
                j = this.circle(j);
                this.loadElement(j);
        ...
        initStartIndex: function () {
            var index = this.options.index, urlProperty = this.options.urlProperty, i;
        ...
        initEventListeners: function () {
            var that = this, slidesContainer = this.slidesContainer, proxyListener = function (event) {
        ...
        destroyEventListeners: function () {
            var slidesContainer = this.slidesContainer, proxyListener = this.proxyListener;
        ...
        initWidget: function () {
            var that = this, openHandler = function (event) {
                    if (event.target === that.container[0]) {
...
define('company/us/index', [
    'require',
    'jquery',
    'common/extra/Gallery/js/blueimp-gallery'
], function (require) {
    var $ = require('jquery');
    function init() {
    }
