...
define([
    ...
    var renderResult = etpl.compile('<h3>${name}</h3>        <table>            <tr>            <th>shape\u7C7B\u578B</th>            <!-- for: ${result} as ${item} -->            <th>${item.shapeType}</th>            <!-- /for -->            </tr>            <tr>            <td>op/s</td>            <!-- for: ${result} as ${item} -->            <td>${item.ops}</td>            <!-- /for -->            </tr>        </table>');
    var shapeList = [];
    var N_SHAPE = 20;
    window.N_ITER = 2000;
    window.points = [];
    ...
    document.getElementById('run').addEventListener('click', function () {
        ...
        window['jsInsidePath'] = function (x, y) {
            for (var j = 0; j < N_SHAPE; j++) {
                var shape = shapeList[j];
                if (area.isInsidePath(shape._pathProxy.pathCommands, shape.style.lineWidth, shape.style.brushType, x, y)) {
                    return true;
                }
            }
        };
        function _isPainted(context, x, y, unit) {
            ...
            if (typeof unit != 'undefined') {
                unit = (unit || 1) >> 1;
                pixelsData = context.getImageData(x - unit, y - unit, unit + unit, unit + unit).data;
            } else {
