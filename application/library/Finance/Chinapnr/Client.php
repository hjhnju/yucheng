...
class Finance_Chinapnr_Client {
    ...
    public static function getInstance(){
        if(self::$self == null){
            ...
            self::$self = new Finance_Chinapnr_Client($arrConf);
        }
        return  self::$self;
    }
     ...
     */
    private function __construct($arrConf){
        $this->platformUrl = $arrConf['serverLocation'];
        $this->merId       = $arrConf['merId'];
        $this->merCustId   = $arrConf['merCustId'];
        $this->privateKey  = $arrConf['merchantPrivateKey'];
        $this->publicKey   = $arrConf['chinapnrPublicKey'];
        $this->scureTool   = new Finance_Chinapnr_SecureTool($this->privateKey,
            $this->publicKey);
    }
...
HTML;
     ...
     * @return Ambigous <multitype:, NULL, mixed>
     */
    public function queryBalanceBg($usrCustId)
    {
        $checkValue= $this->sign($this::VERSION_10.$this::CMDID_QUERY_BALANCE_BG.$this->merCustId.$usrCustId);
